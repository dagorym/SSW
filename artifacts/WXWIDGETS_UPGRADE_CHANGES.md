...
- Subtask 3 of the GUI console warning remediation plan extended that same wxGTK static-box ownership rule across the remaining scoped SSW dialogs: `ViewFleetGUI`, `SystemDialogGUI`, `TransferShipsGUI`, `UPFUnattachedGUI`, `SatharFleetsGUI`, `SelectJumpGUI`, and `BattleResultsGUI` now construct static-box-contained controls with each sizer's `GetStaticBox()` parent instead of `this`.
- Strategic regression coverage for Subtask 3 is intentionally source-inspection based: `StrategicGuiLiveTest` now verifies those seven dialog source files contain the remediated `GetStaticBox()` constructor patterns and reject the legacy `new ... ( this, ...)` anti-pattern for the affected controls, with the `BattleResultsGUI` checks explicitly covering the edit-stats `m_textCtrl2`, `m_staticText8`, and `m_staticText12` constructors under `Edit Ship Statistics`, while the existing live dialog tests continue to exercise the same dialogs' runtime flows.
- Validation outcome for the post-remediation tester rerun: `cd tests/gui && make -s && xvfb-run -a ./GuiTests` passed with 25/25 GUI tests, including the updated `BattleResultsGUI` static-box parentage checks. Known wx debug static-box warnings still appear for out-of-scope `ScenarioEditorGUI` (`"Create a Custom Scenario"`), which remains the later BattleSim-focused remediation scope.
