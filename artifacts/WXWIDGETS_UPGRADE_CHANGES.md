...
- Strategic regression coverage for Subtask 3 is intentionally source-inspection based: `StrategicGuiLiveTest` now verifies those seven dialog source files contain the remediated `GetStaticBox()` constructor patterns and reject the legacy `new ... ( this, ...)` anti-pattern for the affected controls, while the existing live dialog tests continue to exercise the same dialogs' runtime flows.
- Validation outcome for that remediation pass: `cd tests/gui && make -j2 && xvfb-run -a ./GuiTests`, `cd tests/gui && make -j2 && xvfb-run -a ./GuiTests`, `cd tests/gui && timeout 180s xvfb-run -a ./GuiTests`, and `cd tests/gui && make -j2 && timeout 180s xvfb-run -a ./GuiTests` completed with a final `GuiTests` result of `OK (25 tests)`. wx debug static-box warnings still appear for `ScenarioEditorGUI` (`"Create a Custom Scenario"`), which was unchanged here and remains the later BattleSim-focused remediation scope.
