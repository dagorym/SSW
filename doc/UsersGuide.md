...
The entire battle screen window can be resized as desired based on your screen resolution.  The initial size of the window is 1200x900 pixels.  The data display area starts at a compact 120-pixel baseline when space allows, but it can grow when the current tactical controls need more room.  Even when the lower panel grows, the battlefield keeps at least 60% of the window's client height and otherwise expands to fill the remaining space.  While the window is open, dragging a resize edge immediately reflows the lower panel so the movement reminder, selected-ship status block, and completion-button band stay separated as the available width changes.
