...
- Open – Selecting the Open option bring up the Open File Dialog box allowing you to select a previously saved game and load it into the simulator to pick up where you left off.  If you select this option while a game is loaded you will be prompted to save the current game first if desired.  Save files are tagged with a format version; a file that cannot be opened, is corrupt, or was written by an incompatible version of the simulator is rejected with an on-screen error message instead of being partially loaded, and the simulator returns to the same no-game-loaded state as at startup.  As a result, save files written by builds prior to this format change can no longer be opened.
...
You get to the tactical combat map either by selecting Show Battle Board from the Show Menu on the main screen or during a combat encounter in the strategic game by clicking "To the Battle Board!" in the Combat Resolution Method dialog.  In either case you are presented with a screen similar to the one shown in figure 15.  The tactical battle board now includes a native menu bar across the top of the window with File, Settings and Help menus.  In the current build, Load Game, Save Game, Damage Details, User's Guide and About are visible but disabled placeholders, while File->Quit is the active menu command for closing the battle board and returning control to the strategic game or BattleSim caller.
...
The entire battle screen window can be resized as desired based on your screen resolution.  The initial size of the window is 1200x900 pixels.  The data display area starts at a compact 120-pixel baseline when space allows, but it can grow when the current tactical controls need more room.  Even when the lower panel grows, the battlefield keeps at least 60% of the window's client height and otherwise expands to fill the remaining space.  While the window is open, dragging a resize edge immediately reflows the lower panel so the movement reminder, selected-ship status block, and completion-button band stay separated as the available width changes.  When enough width exists, the selected-ship statistics stay split to the right of the prompt area; when the available width is too narrow to show the full statistics block, the display stacks that block below the action-button row instead of clipping long weapon lists or overlapping the controls.  When that width pressure is removed, the lower panel can shrink back toward its compact baseline instead of staying stuck at the largest height it previously needed.
...
To place your station simply click on one of the six hexes around the planet.  Clicking anywhere else has no effect.  Once you have clicked on a location, the station appears on the map and the data display area changes to give you two buttons labeled 'clockwise' and 'counterclockwise' as shown in figure 19.  Until you choose one of those orbital-direction buttons, you can still click a different adjacent hex to relocate the pending station placement; the map keeps only that latest valid station hex.  Click on the button to pick the orbital direction of the station around the planet.  Once you do, the station icon will be oriented so that the left side (if you were looking at it with the name right side up) is pointed in the direction of travel.  So if you selected clockwise, the station name will be on the side opposite the planet and if you selected counterclockwise, the station name will be next to the planet.  Once your station is set up, it is time to set up your fleets.
...
If the defending force still has deployable mines or seeker missiles after ship setup, the data display switches to an ordnance-placement panel before tactical play begins.  Each mine or seeker launcher appears as its own clickable source row, including the ship name, weapon type/slot, and current ammo for that exact launcher.  Click a row to choose the active source, then click a legal hex on the map to place that mine or seeker.  Setup markers are tinted by source so each ship-and-launcher combination keeps a stable distinct color during setup.
During the mine-placement step (`BS_PlaceMines`), clicking a hex that already contains a mine removes it and restores the ammo to the ship that originally placed it — even if you have a different source selected at the time.  During the seeker-placement step (`BS_PlaceSeekers`), a centered recall list appears below the source-selection rows, showing one row per placed (hex, source-ship) group labeled with the ship name, hex coordinates, and the stack count.  Clicking a recall row removes exactly one seeker from that group and restores one round of ammo to the placing ship.  The recall list is only visible during seeker placement; it does not appear during the mine step.  Press **Mine Placement Done** or **Seeker Placement Done** to close each placement step.
Note:  Planet, station, ship position, heading, and initial-speed decisions are still final once accepted.  The only setup-stage undo currently available is the mine/seeker placement step described above, and that undo window ends when you press the relevant Done button.

If the moving player begins a turn with inactive seeker missiles already on the board, tactical play pauses for a seeker activation step before normal ship movement.  During this stop the battle map shows only that moving player's inactive seeker stacks; active seekers are hidden until regular battle rendering resumes.  Click a visible seeker stack to select its hex, then use the lower panel's per-seeker rows to activate individual seekers one at a time.  Each row represents one seeker in the selected stack, so multiple same-hex seekers remain separately selectable.  When you are finished, click **Seeker Activation Done** to resolve that moving player's active seeker movement before any ship movement begins: seekers already sharing a ship hex detonate immediately, seekers that contact a ship during their autonomous move stop on that contact hex, and seekers that reach their 12-hex movement turn without contacting a ship expire.  After that resolution step, normal movement begins and only active seekers remain visible on the map.
If a seeker detonates in a hex containing more than one ship, the game automatically attacks the ship in that hex with the highest original hull strength; ties among equally large ships are broken randomly.  The normal ICM allocation dialog is still used for interception, and any resulting immediate damage summary is labeled **Seeker Damage** so seeker detonations stay distinct from mine-damage reports before destroyed ships are removed from the board.
...
If a ship starts the phase at speed 0 and still has MR remaining, the highlighted preview routes represent all six legal starting facings it may choose before it moves, including the straight-ahead route in its current heading.  You can still click an adjacent highlighted facing hex to set the ship's facing first, but you may also click any highlighted preview-route hex directly; the simulator infers the starting facing from that preview and immediately continues the normal route-selection flow from the chosen direction.  After that first choice, route trimming and extension work the same way they do for any other move, or you can leave the ship in its current hex and use the Movement Done flow to rotate in place for the turn.
...
*Figure 25: Activated 'Movement Done' button after all of your ships have satisfied their movement requirements.*

However, you must eventually complete each ship's legal movement before moving on.  For ships that are actually traveling, that means getting them into the orange ending hex zones.  For ships that began the turn stopped, that can also mean selecting a new facing and ending the turn in the same hex.  Once you have done that for all of your ships, the Movement Done button in the data display area illuminates (figure 25).  Once you are satisfied with your movement orders, select this button.  Your ships will be moved to the ends of the movement lines, any rotate-in-place facing changes will be committed, and it is now time for defensive fire.  The movement lines will stay on the board so that the opposing player can see where you traveled during the last turn.

### End-of-move facing change (Turn Left / Turn Right)

After a ship has satisfied its minimum movement requirement and still has MR remaining, two additional buttons appear in the data display area to the right of the Movement Done button: **Turn Left** and **Turn Right**.  A short caption above the two buttons ("If at the end of a ship's movement you want to make a facing change (and have remaining MR), use these buttons to make a single final turn.") reminds you of the rule below.  Clicking either button rotates the selected ship one hex-facing in that direction, consuming 1 MR.  This facing change is a reversible preview — it appears immediately on the battle map so you can see the new heading, but it is not committed to the game until you click **Movement Done**.  To undo a facing change, simply press the opposite button: pressing **Turn Right** after a **Turn Left** (or vice versa) returns the ship to its original heading with no net change.  Only one hexside of rotation is available per turn regardless of remaining MR.  Clicking any movement hex on the board while a pending facing change is active cancels the turn and restores the original heading before processing the hex selection.  Ships that decelerated to speed 0 this turn may also use the end-of-move facing change.
...
During the moving player's offensive fire phase, seeker missile (`SM`) launchers are a special case.  Selecting an `SM` launcher does not enter normal target selection.  Instead, the battle map highlights the legal deployment hexes on that ship's current-turn path (start hex, traversed hexes, and final/current hex), and each click on one of those highlighted hexes deploys one inactive seeker there immediately and reduces that launcher's ammo by one.  Repeated clicks on the same highlighted hex continue stacking additional pending seekers in that hex until the launcher runs out of ammo.
...
While an `SM` launcher remains selected, the lower panel lists pending seeker deployments grouped by hex.  Clicking a recall row removes exactly one current-phase pending seeker from that grouped hex and restores one shot to that same launcher.  This recall window applies only to seekers deployed by the selected launcher during the current offensive fire phase; seekers that were placed during setup or on earlier turns are not recalled or highlighted by this panel.  When you press **Offensive Fire Done**, any remaining pending seeker deployments are committed and remain on the board as ordinary inactive seekers for later activation.
