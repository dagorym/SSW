...
If a ship starts the phase at speed 0 and still has MR remaining, the adjacent highlighted hexes first represent the legal facings it may choose before it moves.  Click one of those adjacent hexes to set the ship's facing, then either continue plotting a route from that facing or leave the ship in its current hex and use the Movement Done flow to rotate in place for the turn.
