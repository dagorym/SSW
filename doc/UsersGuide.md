...
If the moving player begins a turn with inactive seeker missiles already on the board, tactical play pauses for a seeker activation step before normal ship movement.  During this stop the battle map shows only that moving player's inactive seeker stacks; active seekers are hidden until regular battle rendering resumes.  Click a visible seeker stack to select its hex, then use the lower panel's per-seeker rows to activate individual seekers one at a time.  Each row represents one seeker in the selected stack, so multiple same-hex seekers remain separately selectable.  When you are finished, click **Seeker Activation Done** to continue into the normal movement phase, where only active seekers are shown on the map.
