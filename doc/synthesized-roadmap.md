...
| C2 | **Retreat conditions 4 and 5 evaluated swapped** | `src/strategic/FGame.cpp:1046-1064` vs `:980-994` | The display (and `SatharRetreatGUI` in doc order) says 4 = "tenday without destroying two stations", 5 = "Sathar lost more ships". `checkForVictory()` implements case 4 as the ship-loss comparison and case 5 as the station check. Whichever condition the player picks, the other is enforced. **[RESOLVED — see CRIT-2: `checkForVictory()`'s case-4 and case-5 bodies are swapped so case 4 evaluates the station-destruction condition (`m_stationsDestroyed`) and case 5 evaluates the ship-loss condition (`m_lostTendaySathar`/`m_lostTendayUPF`), matching `showRetreatConditions()`'s displayed text.]** |
| C3 | **Dangling `m_currentDefense` after `FVehicle::load()`** | `src/ships/FVehicle.cpp:218-228` | `load()` deletes every defense and rebuilds the list but never reassigns `m_currentDefense`, which still points at the deleted constructor-time `m_defenses[0]`. Every post-load consumer (`FWeapon::fire()`, `FBattleBoard.cpp:276`, `FBattleDisplay.cpp:1535`, `FTacticalGame.cpp:3360`) dereferences freed memory — bites whenever a loaded game enters tactical combat. Related: the active defense selection was not persisted at all (H2 below, since resolved by P5-2). **[RESOLVED — see CRIT-3: `FVehicle::load()` re-points `m_currentDefense` at the live `m_defenses[0]` entry (or a fresh default `FNone` fallback) after the defense list is rebuilt from the stream.]** |
| C4 | **Out-of-bounds planet index from `TwoPlanetsGUI`** — ***RESOLVED*** (CRIT-4, commit `bf9dbec0`) | `include/gui/TwoPlanetsGUI.h:40-41` + `src/gui/SelectCombatGUI.cpp:217,227,258,273` | Dialog returns `EndModal(1)`/`EndModal(2)`; caller does `planet = d.ShowModal()` then indexes `getPlanetList()[planet]` on a 2-element (0/1) list. Button 1 attacks the *wrong* planet; button 2 indexes `[2]` — UB/crash. _Resolved: `SelectCombatGUI::onAttack()` now translates the `ShowModal()` return into a 0-based index (button 1 → 0, button 2 → 1, any other/cancel value → 0) before all three `getPlanetList()[planet]` call sites; `include/gui/TwoPlanetsGUI.h` was left unmodified per its generated-file banner; see `artifacts/phase1-critical-defects/CRIT-4`._ |
| C5 | **"Yes, save" on close never saves** | `src/FMainFrame.cpp:298-308` | Dialog is `wxYES_NO\|wxCANCEL` but the code tests `result == wxID_OK`; `ShowModal()` returns `wxID_YES`. Answering Yes silently discards the game. **[RESOLVED — see CRIT-5: `FMainFrame::onClose()` now compares against `wxID_YES`, so choosing "Yes, save" invokes `onSave()` before `resetGame()`; No still resets without saving and Cancel still aborts the close with no save and no reset.]** |
| C6 | **Unchecked `readlink` — latent out-of-bounds write** | `src/core/FGameConfig.cpp:111-113` | `readlink` returns `ssize_t`; on failure −1 stored into `size_t` makes `buf[size] = 0` a wild write; a ≥1000-char path writes one past the array. Runs in a singleton constructor with no error path. **[RESOLVED — see CRIT-6: `FGameConfig::computeSafeTerminatorIndex()` clamps the terminator index and a readlink failure now falls back to an empty path state instead of writing out of bounds.]** |

### High

| # | Defect | Location | Detail |
|---|--------|----------|--------|
| H1 | **Save format: no magic, no version, no error detection, non-portable widths** — ***RESOLVED*** (Phase 5 Save-Format Hardening, `artifacts/phase5-save-format-hardening/`) | `include/core/FPObject.h:93-132`, `src/core/FPObject.cpp:28-40`, `src/strategic/FGame.cpp:569-591` | Every `read`/`write` unconditionally returns 0 regardless of stream state (the documented error-code contract is implemented nowhere); `readString` allocates an unchecked size (corrupt file → multi-GB alloc or garbage); an unknown ship type makes `createShip` return NULL and `FFleet::load` immediately calls `v->load(is)` — null-deref on a bad save. Counts are written as `size_t` (8 bytes Linux x64, 4 bytes Win32) — saves are not portable between the two supported platforms. **[RESOLVED — see P5-1 (`include/core/FSaveFormat.h`'s `kSaveMagic`/`kSaveFormatVersion`/`kMaxSerializedStringBytes` constants, the fixed-width little-endian `FPObject::writeU32`/`readU32` primitives, and the capped, checked `writeString`/`readString`), P5-2 (`FVehicle::save()`/`load()` fixed-width IDs/counts, `createShip` NULL-result propagated as a load failure instead of dereferenced), P5-3 (`FPlayer`/`FFleet` fixed-width IDs/counts and load-error propagation), P5-4 (`FMap`/`FSystem`/`FJumpRoute` fixed-width IDs/counts and load-error propagation, including the `FJumpRoute` start/end system ID fields), and P5-5 (`FGame::save()` writes the `kSaveMagic`/`kSaveFormatVersion` header via `writeU32` before any game data and encodes its own counts/IDs with `writeU32`; `FGame::load()` validates the magic and version first and checks every subsequent header read, `m_universe->load()`, and each player's `load()`, reporting the first failure via the installed `IStrategicUI` (console fallback otherwise) and aborting with no half-built game committed as the live singleton; `FMainFrame::onOpen()` tears a freshly-created failed game back down via `resetGame()` without enabling turn/menu items). The one remaining non-portability item — weapon/defense `m_type` enum ordinals — is tracked separately as the `SF-enum-codes` deferred item (`doc/deferred-tasks.md`), since it is a stable-serialization-code concern rather than a fixed-width count/ID concern.]** |
| H2 | **Active defense not persisted; MS state inconsistent after load** — ***RESOLVED*** (P5-2, `artifacts/phase5-save-format-hardening/P5-2`) | `src/ships/FVehicle.cpp:154-180` | `save()` writes `m_maskingScreenTurnCount` but not which defense is current; a ship saved with a masking screen up reloads with RH active and a nonzero MS count. **[RESOLVED — see P5-2: `FVehicle::save()` writes the index of `m_currentDefense` within `m_defenses` as a trailing field, and `FVehicle::load()` restores `m_currentDefense` to that saved index after the defense list is rebuilt (falling back to index 0, or a fresh default `FNone`, when the index is invalid or the list is empty, preserving the CRIT-3 safety fallback); behavioral coverage in `FVehicleTest::testLoadRestoresActiveDefenseSelectionAcrossSaveLoad`.]** |
| H3 | **ID counters not restored on load — post-load collisions** — ***RESOLVED*** (P5-2, P5-3, P5-4, `artifacts/phase5-save-format-hardening/`) | `FVehicle.cpp:188` (`m_nextID` never advanced), same pattern in `FFleet`, `FPlayer`, `FSystem`, `FJumpRoute` | Ships/fleets created after a load can duplicate loaded IDs. Tactical bookkeeping is keyed by ship ID; `removeFleet(id)` can remove the wrong fleet. Fix: `if (m_ID >= m_nextID) m_nextID = m_ID + 1;` in each `load()`. **[`FVehicle` instance resolved — see P5-2: `FVehicle::load()` now advances the static `m_nextID` past any loaded `m_ID` using this guard, with behavioral coverage in `FVehicleTest::testLoadAdvancesNextIDPastLoadedID`. `FFleet`/`FPlayer` instances also resolved — see P5-3: `FFleet::load()` and `FPlayer::load()` apply the same guard to their respective `m_nextID` statics, with behavioral coverage in `FFleetTest::testLoadAdvancesNextIDPastLoadedID` and `FPlayerTest::testLoadAdvancesNextIDPastLoadedID`. The `FSystem`/`FJumpRoute` instances of this same pattern are also resolved — see P5-4: `FSystem::load()` and `FJumpRoute::load()` apply the same guard to their respective `m_nextID` statics, with behavioral coverage in `FSystemTest::testLoadAdvancesNextIdPastLoadedID` and `FJumpRouteTest::testLoadAdvancesNextIdPastLoadedID`. All five instances of this pattern are now resolved.]** |
| H4 | **`NO_ROUTE` legacy normalization destroys jump route 0** — ***RESOLVED*** (P5-3, `artifacts/phase5-save-format-hardening/P5-3`) | `src/strategic/FFleet.cpp:158-159`, `FJumpRoute.cpp:15,43` | Route IDs start at 0; the first route created ("Sathar Start Circle 2" → Prenglar) has ID 0, so any fleet saved mid-transit on it loses its route on load and is stuck forever. **[RESOLVED — see P5-3: the `if (m_jumpRouteID == 0) m_jumpRouteID = NO_ROUTE;` normalization in `FFleet::load()` was removed, so a jump route ID of 0 is preserved exactly as saved and only the distinguished `NO_ROUTE` sentinel value means "not on a route"; behavioral coverage in `FFleetTest::testJumpRouteZeroPreservedAcrossSaveLoad`.]** |
...
| H7 | **`wxClientDC` drawing inside the paint path** — ***RESOLVED*** (P4-2, `artifacts/phase4-tactical-gui-hybrid-cleanup/P4-2`) | `src/tactical/FBattleBoard.cpp:249` (called from `onPaint` via `drawShips`/`drawSeekerMissiles`); also `src/FGamePanel.cpp:20-24` | Half the tactical scene renders through the `wxPaintDC`, half through per-image `wxClientDC`s in the same pass. Unsupported/ignored on wxGTK3-Wayland and wxOSX, undefined z-order, bypasses the update-region clip. The single biggest portability landmine in the GUI. _Resolved: `FBattleBoard::onPaint()` constructs a single `wxAutoBufferedPaintDC` (valid because the constructor now calls `SetBackgroundStyle(wxBG_STYLE_PAINT)`) and threads it through `draw(wxDC&)` → `drawShips(wxDC&)` → `drawCenteredOnHex(wxDC&, wxImage, FPoint, int)` and through `drawSeekerMissiles(wxDC&)`, so the entire scene (grid, planet, ships, seekers, routes, ranges, targets, mines, overlays) renders in one buffered pass with no `wxClientDC` left anywhere in `FBattleBoard.cpp`; `FGamePanel::onPaint()` now uses a single `wxPaintDC` on all platforms, removing the Linux-only `wxClientDC` branch and the trailing dead commented-out draw block. Behavioral coverage: `FTacticalBattleBoardRendererDelegationTest`'s 5 source-contract literals were updated to the new dc-threaded signatures, and `TacticalGuiLiveTest::testSeekerPathRendersInPHMoveWithMovementPath` was narrowed to a cyan-pixel-count assertion isolating the seeker path overlay now that the seeker icon itself correctly renders in that phase; pass-2 remediation added `TacticalGuiLiveTest::testShipAndPlanetIconsRenderThroughCallerSuppliedDC`, a direct behavioral offscreen-`wxMemoryDC` test that places a ship and a planet at known hexes, calls `FBattleBoard::draw(dc)` directly, and asserts pixels changed in the expected hex-centered regions — closing the pass-1 Verifier's BLOCKING finding that acceptance criterion 5 rested only on source-contract literals and manual diff review; see `artifacts/phase4-tactical-gui-hybrid-cleanup/P4-2`._ |
...
| H9 | **`m_first` flag drives connect/show lifecycle of seven buttons** — ***RESOLVED*** (P4-1, `artifacts/phase4-tactical-gui-hybrid-cleanup/P4-1`) | `include/tactical/FBattleDisplay.h:111` + 7 sites in `FBattleDisplay.cpp` | Any phase transition that bypasses the matching Done handler (model-driven transitions exist) leaves the next phase's button never connected/shown — a soft-lock with no crash. Fix: Bind all handlers once in the ctor; drive only Show/Enable per phase. _Resolved: `m_first` is removed from `FBattleDisplay`; all seven phase Done/Set-Speed buttons and both turn buttons bind their click handlers exactly once in the constructor; a new non-paint `FBattleDisplay::updateForPhase()` is the single seam that drives each button's `Show()`/`Enable()` and the turn-panel state from `FBattleScreen::getState()`/`getPhase()` and model queries, invoked from `FBattleScreen::setState()`/`setPhase()`/`onSize()`/`reDraw()`; no phase transition can now leave a button unconnected or unshown. Behavioral GUI coverage added in `TacticalGuiLiveTest` (5 new tests) plus 6 re-authored source-contract supplements in `FTacticalBattleDisplayFireFlowTest`; see `artifacts/phase4-tactical-gui-hybrid-cleanup/P4-1`._ |
...
- `FPlayer::m_destroyed` never freed, never serialized (it is the data the Replacements rule
  needs). **[RESOLVED — see P2-5: `FPlayer::~FPlayer()` now deletes and clears every
  ship in `m_destroyed`, with the sole-ownership contract documented on `addDestroyedShip()`
  and `m_destroyed`. Serialization resolved by P5-3 (`artifacts/phase5-save-format-hardening/P5-3`):
  `FPlayer::save()`/`load()` now persist and restore `m_destroyed` using the same
  type-tag + `createShip()` + `FVehicle::load()` pattern as `m_unattached` and fleet
  ships; the list round-trips but is not yet consumed by any rule. The
  `doc/deferred-tasks.md` `F2-serialization` entry has been removed as resolved.]**
...
5. Save-format hardening (version header, fixed-width, error propagation, persist current
   ...
   **[RESOLVED — see Phase 5 Save-Format Hardening (`artifacts/phase5-save-format-hardening/`):
   P5-1 added the shared wire-format primitives and constants (`include/core/FSaveFormat.h`,
   `FPObject::writeU32`/`readU32`/`writeString`/`readString`); P5-2 hardened `FVehicle`
   (fixed-width IDs/counts, persisted active-defense selection, `m_nextID` restoration); P5-3
   hardened `FPlayer`/`FFleet` (fixed-width IDs/counts, `m_nextID` restoration, the `NO_ROUTE`
   legacy-normalization removal, and `m_destroyed` serialization); P5-4 hardened
   `FMap`/`FSystem`/`FJumpRoute` (fixed-width IDs/counts, `m_nextID` restoration, and real
   fixed-width `FJumpRoute` start/end system ID fields replacing the pointer-smuggling hack);
   and P5-5 added the `FGame`-level `kSaveMagic`/`kSaveFormatVersion` header, fixed-width
   `FGame` counts/IDs, aggregated `load()` error propagation reported via `IStrategicUI`, and
   the `FMainFrame::onOpen()` abort/preserve/report loader UX. See H1, H2, H3, and H4 above for
   the per-defect resolution detail. Deliberately scoped out and tracked separately in
   `doc/deferred-tasks.md`: `SF-enum-codes` (stable weapon/defense type codes) and
   `SF-fhexmap` (tactical hex-occupancy persistence).]**
