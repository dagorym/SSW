...
void ICMSelectionGUI::selectWeapon(int row){
	...
	for (unsigned int i = 0; i< m_currentRowData->vehicles->size(); i++){
		...
		if ((*vList)[i]->getOwner() == m_currentRowData->weapon->getTarget()->getOwner()){
			...
			m_ICMsApplied.push_back(new wxSpinCtrl( m_panel1, count, wxEmptyString, wxDefaultPosition, wxDefaultSize, wxSP_ARROW_KEYS | wxSP_VERTICAL, 0, maxICMs, ICMdata->getICMsAllocatedToWeapon(m_currentRowData->weapon) ));
