/**
 * @file FMainBattleSimFrame.cpp
 * @brief Implementation file for FMainBattleSimFrame class
 * @author Tom Stephens
 * @date Created:  Feb 28, 2005
 *
 */
#include "BattleSim/FMainBattleSimFrame.h"
...
FMainBattleSimFrame::~FMainBattleSimFrame() {
	if(m_game != NULL){
		delete m_game;
	}
	delete m_gameConfig;
...
FMainBattleSimFrame::FMainBattleSimFrame(const wxString& title, const wxPoint& pos, const wxSize& size)
       : wxFrame((wxFrame *)NULL, -1, title, pos, size) {

	m_novaPlaced = false;
	this->Connect(wxEVT_PAINT, wxPaintEventHandler(FMainBattleSimFrame::onPaint));
...
void FMainBattleSimFrame::onQuit(wxCommandEvent& WXUNUSED(event)) {
    Close( TRUE );
}

void FMainBattleSimFrame::onNew(wxCommandEvent& event) {
	if(m_game==NULL){
...
void FMainBattleSimFrame::onSave(wxCommandEvent& WXUNUSED(event)) {
...
void FMainBattleSimFrame::onOpen(wxCommandEvent& event) {
...
void FMainBattleSimFrame::onShowPlayers(wxCommandEvent& WXUNUSED(event)) {
...
void FMainBattleSimFrame::onAbout(wxCommandEvent& WXUNUSED(event)) {
	wxString body = "Star Frontiers Second Sathar War Simulation";
	body += "\n\nWritten by Tom Stephens";
	body += "\n\nCopyright 2005-2010 - New Frontier Games";
	body += "\n\nVersion 0.0.11";
...
void FMainBattleSimFrame::onPaint(wxPaintEvent & event){
	wxPaintDC dc(this);
	if (m_game!=NULL){
		m_game->draw(dc);
	}
...
void FMainBattleSimFrame::onEndUPFTurn(wxCommandEvent& event){
    if(m_game != NULL){
    	m_game->endUPFTurn();
        m_game->draw();
        GetMenuBar()->GetMenu(2)->FindItemByPosition(0)->Enable(true);
...
void FMainBattleSimFrame::onEndSatharTurn(wxCommandEvent& event){
    if(m_game != NULL){
...
void FMainBattleSimFrame::onLeftDClick(wxMouseEvent& event) {
    if(m_game != NULL){
    	m_game->onLeftDClick(event);
    }
}

void FMainBattleSimFrame::onPlaceNova(wxCommandEvent& event){
    if(m_game != NULL){
...
void FMainBattleSimFrame::onLeftUp(wxMouseEvent& event){
	if(m_game != NULL){
...
void FMainBattleSimFrame::onSize(wxSizeEvent& event){
	if(m_game != NULL){
		Refresh();
	}
}

void FMainBattleSimFrame::onClose(wxCommandEvent &event){
...
void FMainBattleSimFrame::resetGame(){
	delete m_game;
	m_game = NULL;
	m_novaPlaced = false;
...
void FMainBattleSimFrame::onAddSatharShips(wxCommandEvent& event){
	if(m_game!=NULL){
		m_game->onAddSatharShips();
	}
}

void FMainBattleSimFrame::onShowRetreatConditions(wxCommandEvent& event){
	if (m_game!=NULL){
		m_game->showRetreatConditions();
	}
}

void FMainBattleSimFrame::onShowBattleScreen(wxCommandEvent& event){
