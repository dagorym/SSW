...
- `framework-discovery` for selecting the existing test framework, conventions, and smallest relevant commands.
- `test-authoring` for deciding whether to keep, add, or update tests and for writing coverage-oriented test changes.
- `test-execution` for running tests and producing structured result summaries.
...
- `commit-flow` for tester-specific commit decisions and commit ordering.
- `artifact-writing` for exact tester success and failure artifact requirements.
...
3. Prefer existing tests when they already provide sufficient coverage.
4. Add new tests only for uncovered acceptance criteria or additional edge cases introduced by the implementation.
5. Update existing tests only when an expected regression or intentionally changed behavior requires the test to change; treat changes to existing tests as a last resort and a potential sign that the implementation may have broken prior behavior.
6. Use the existing testing framework consistent with the project's test configuration.
7. Execute tests and report results with structured output.
8. Never modify implementation code.
9. Treat required final artifact files as explicitly permitted outputs even when those artifact paths are outside the specified test directories.
10. Stop after 3 attempts if tests continue to fail.
11. Report which acceptance criteria are not met and why.
12. When testing passes, produce a documenter-agent prompt that includes all context needed to continue without asking the user for more information.
13. When the tests are valid and should be handed off, commit the test changes first, then capture the resulting test commit hash before writing required output artifacts.
14. Write machine-readable handoff artifacts to the shared artifact directory only after the test commit hash has been captured, then commit those artifacts in a second commit. If no test commit is made, write "No Changes Made" to the artifact JSON instead of a commit hash.
...
7. If the run reaches a repeated-failure stop condition, invalid-test-generation outcome, or confirmed unmet acceptance criteria, load `failure-reporting`, stop further attempts, and emit the required failure output.
