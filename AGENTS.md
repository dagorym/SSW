...
- For wxGTK dialog constructors that rely on sizers, follow `Layout()`/`Fit(this)` by setting the dialog minimum size from `GetBestSize()` and centering it on its parent (or on screen when unparented) so first-show geometry does not clip controls or action buttons.
