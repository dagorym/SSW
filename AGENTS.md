...
- Keep the active runtime tactical GUI flow on `FBattleScreen`, `FBattleBoard`, and `FBattleDisplay` until later milestones explicitly rewire delegation; Milestones 5-8 documentation should describe `FTacticalGame`, `ITacticalUI`, and `WXTacticalUI` as migration infrastructure for the live wx runtime path rather than a fully separate replacement. Milestone 8 Subtask 1 expanded the forwarding surface for selection, placement, hex-click handling, movement/fire progression, and renderer-facing state queries; Milestone 8 Subtask 3 then narrowed `FBattleBoard` to geometry/rendering/scrolling/hit-testing by routing clicks through `FBattleScreen::handleHexClick()` to `FTacticalGame` and reading occupancy/path/range/mine overlays back through model-backed accessors. `FBattleDisplay` is still on the live wx path and has not yet been reduced to a pure translator.
