...
- On wxGTK, prefer `wxDefaultSize` plus `SetMinSize(GetBestSize())` for sizer-managed `wxSpinCtrl` controls instead of narrow fixed widths so spin buttons keep valid geometry after layout.
