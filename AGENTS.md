...
- GUI tests: `tests/gui/*` (covers live wx dialog/widget behavior including `StrategicGuiLiveTest`, `TacticalGuiLiveTest`, deterministic modal driving through the shared `WXGuiTestHarness` action helpers, parent-backed `WXTacticalUI` redraw/message/damage-summary/winner smoke flows, `TacticalDamageSummaryGUI` live dialog coverage, and model-state assertions for strategic dialogs such as fleet setup, ship transfer, combat selection, and battle results flows)
