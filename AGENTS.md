...
- GUI tests: `tests/gui/*` — `StrategicGuiLiveTest`, `TacticalGuiLiveTest`, and `BattleSimGuiLiveTest` using `WXGuiTestHarness` helpers (`showModalWithAction(...)`, `runModalFunctionWithAction(...)`, `runVoidFunctionWithAction(...)`, `waitForTopLevelWindow(...)`, `waitForTopLevelWindowClosed(...)`, `waitForModalDialog(...)`). Key invariants:
  ...
  - Posted `ID_TacticalQuit` and `wxEVT_CLOSE_WINDOW` close vectors must leave battle screens hidden or absent; harness waits drive wx idle cleanup until windows disappear. When one or more `WXTacticalUI` child modal dialogs (damage summary, ICM selection, or winner/info message) are active at close time, `closeBattleScreen()` calls `dismissActiveDialog()` on the owned `WXTacticalUI` before `Hide()`/`Show(false)` to prevent the duplicate `wxDialog::EndModal` assert in wxGTK (`src/gtk/dialog.cpp:193`). `WXTacticalUI` exposes `hasPendingDialog()` (returns `true` while any tracked dialog is still modal) and `dismissActiveDialog()` (calls `EndModal(wxID_CANCEL)` on every tracked dialog still modal, innermost-first, to clear each `IsModal()`) as the supporting seam; since TMFR-01, the tracked dialogs live in `m_dialogStack` (`std::vector<wxDialog*>`, replacing the earlier single `m_activeDialog` pointer so more than one live child dialog can all be dismissed by one `dismissActiveDialog()` call), with each dialog pushed onto the stack immediately before its own `ShowModal` call in `showMessage`, `showDamageSummary`, or `runICMSelection` and popped immediately after that call returns. In tests, `wxEVT_CLOSE_WINDOW` is posted after `runVoidFunctionWithAction` returns (not from within the `ShowModal` action lambda) to avoid a `Destroy()`-during-`ShowModal` double-free that is specific to the non-modal test setup (a harness artifact absent in production, where `FBattleScreen` runs via `ShowModal` and `closeBattleScreen()` calls `EndModal` with no `Destroy()`); `hasPendingDialog()`/`dismissActiveDialog()` are driven directly from the action lambda to verify dismiss-API correctness while `ShowModal` is live, and a source-contract supplement locks the `dismissActiveDialog()` call in `closeBattleScreen()` to cover the integration point that cannot be reproduced in-harness without triggering the double-free. As of TMFR-01, `FBattleScreen::ShowModal()`/`EndModal()` preserve battle modality with a `wxWindowDisabler` (member `m_windowDisabler`, disabling every other top-level window for the battle's duration and restoring them on delete) rather than a GTK `gtk_window_set_modal()` grab, so the frame's `wxMINIMIZE_BOX` and title-bar close control remain effective; see `doc/test-contracts.md`'s TMFR-01 entry for the automated coverage of this mechanism and its documented manual-verification gaps.
