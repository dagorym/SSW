...
- Tactical tests: `tests/tactical/*` (covers `FTacticalGame` header inclusion and additive mechanics ownership via `FTacticalGameMechanicsTest`, `FTacticalCombatReport`, `FTacticalAttackResult`, `ITacticalUI`/`WXTacticalUI` adapter coverage, deterministic mock-`ITacticalUI` winner coverage in `FTacticalGameMockUITest`, stable `WXTacticalUIAdapterTest` behavioral checks for interface construction, null-parent redraw/message/winner fallback safety, and guarded ICM-selection validation without stale source-token or runtime non-rewire assertions, battery range clamping, station orbital movement, the destroyed-ship cleanup lifecycle seam in `FBattleScreen.cpp`, behavioral `FTacticalGame` report-lifecycle/normalization coverage plus hit-detail semantics that only surface per-attack internal events, multi-line `FTacticalCombatReportSummary` ship-rollup formatting including exact `ADF (-N)` / `MR (-N)` bullets plus grouped weapon/defense-hit abbreviations, and source-inspection checks that `FBattleDisplay` constructs its speed spin control with explicit `wxSP_ARROW_KEYS | wxSP_VERTICAL` style plus default positioning and `SetMinSize(GetBestSize())` under a sizer-managed layout rather than the legacy absolute-positioned, hard-coded-width path, loads the tactical zoom image through `resolveAssetPath("data/zoom.png")` instead of raw base-path concatenation, exposes the shared `ACTION_PROMPT_*` spacing constants plus helper seams that reserve prompt-text height before the tactical action-button row, applies that contract across move/defensive-fire/offensive-fire/mine-placement prompt drawing, and `testPlaceMinesSourceListStartsAtActionButtonRowBottom` source-inspects that `drawPlaceMines()` initializes its source-list `y` from `getActionButtonRowBottom()` rather than `BORDER` so the placement source list starts below the instruction/button region with no vertical overlap (SMC-04), that `m_shipNameRegions` entries use the same `y` variable so hit regions stay aligned with drawn text, and that the other lower-panel draw methods (`drawMoveShip`, `drawDefensiveFire`, `drawAttackFire`) do not use `getActionButtonRowBottom()` as their y initializer; relayouts those completion buttons immediately after their show/hide visibility changes, and locks the tactical menu-bar contract through shared menu command IDs, `wxMenu`/`wxMenuBar` creation order, `SetMenuBar(...)` installation, and the rule that only `Quit` binds `wxEVT_MENU`, requests `Close(true)`, and relies on `onClose(...)` plus `closeBattleScreen(GetReturnCode())` as the shared accepted close path without `exit(...)`; the tactical source-contract close regressions also reject treating `IsBeingDeleted()` alone as a successful close and reject `event.Skip()` after the shared close helper has accepted the event; `FTacticalMineDamageFlowTest` now also covers TSM-010 movement-path seeker contact behavior through source-inspection checks that `completeMovePhase()` clears pending outcomes at the start, calls `checkForActiveSeekersOnPath` per moving ship before mine checking, and sequences `applyMovementSeekerDamage()` before `applyMineDamage()` and the defensive-fire phase transition, and that `checkForActiveSeekersOnPath` guards inactive seekers with an explicit `!active` early-continue and `applyMovementSeekerDamage` removes each detonated seeker exactly once; `FTacticalSeekerMovementTest` covers the same TSM-010 contract at runtime by placing inactive and active opposing seekers on a ship's movement path and verifying that inactive seekers generate no contact outcomes, active seekers generate one outcome and are removed from the model after `applyMovementSeekerDamage`, and bystander seekers not on the path survive the detonation pass; `FTacticalBattleBoardRendererDelegationTest` now also covers SMC-07 source contracts: `testDrawSeekerMissilesUsesActivationAndBattleVisibilityRules` verifies the heading-rotated `drawCenteredOnHex(*m_seekerMissileIcon, hex, heading)` call form used for active seekers in normal battle phases, and `testDrawSeekerPathsIsCalledInMovePhaseWithCyanPen` confirms that `drawSeekerPaths` is nested inside the `PH_MOVE` guard in `draw()`, reads seeker records via `m_parent->getSeekerMissiles()`, filters to active seekers with `movementPath.size() >= 2`, and draws stepped lines using a cyan pen; `FTacticalSeekerMovementTest::testMovementPathPopulatedByResolveActiveSeekers` covers the SMC-06 runtime model contract by verifying that `movementPath` is non-empty after `resolveActiveSeekersForMovingPlayer()`, that `movementPath[0]` equals the pre-move starting hex, that `movementPath.back()` matches the final hex, that non-moving seekers have their path cleared, and that a second resolution pass starts fresh from the current hex with no cross-turn accumulation; `FTacticalBattleBoardRendererDelegationTest` also covers the SMC-05 source contract via `testDrawSeekerMissilesPendingOffensiveFireBranch`: the test asserts that `drawSeekerMissiles` contains a `PH_ATTACK_FIRE` branch, calls `m_parent->getAllPendingOffensiveFireSeekerHexes()` to source pending-seeker hexes, draws each in-bounds pending hex without rotation via `drawCenteredOnHex(*m_seekerMissileIcon, *itr)`, draws committed active seekers with heading rotation via `m_parent->getSeekerMissilesAtHex(hex, true)`, and exits early with `return;` so the outer battle-phase loop is skipped; the delegation chain is also verified: `FBattleScreen::getAllPendingOffensiveFireSeekerHexes()` exists in `src/tactical/FBattleScreen.cpp` and delegates to `m_tacticalGame->getAllPendingOffensiveFireSeekerHexes()`, and both `FTacticalGame` and `FBattleScreen` headers declare the method as `const std::vector<FPoint> getAllPendingOffensiveFireSeekerHexes() const` so the model accessor is read-only and wx-free.)
...
- `FBattleBoard::drawSeekerMissiles()` now contains a `PH_ATTACK_FIRE` branch (SMC-05) that draws the seeker icon on every hex holding a current-phase pending offensive-fire seeker, sourced through the read-only delegation chain `FBattleScreen::getAllPendingOffensiveFireSeekerHexes()` → `FTacticalGame::getAllPendingOffensiveFireSeekerHexes()`; pending hexes are drawn without rotation (no heading yet), committed active seekers are drawn with heading rotation, and the branch returns early so the outer battle-phase active-seeker loop is skipped; recalling a pending seeker via the lower-panel recall list removes its icon on the next redraw because the model record is deleted before the redraw occurs; no wx types are introduced into the model by this query.
