...
- Treat `include/tactical/FTacticalGame.h` as the additive non-wx tactical model surface for Milestones 5-8; use it for model-owned tactical mechanics state, ownership bookkeeping types such as `FTacticalHexData` and `FTacticalTurnData`, and the delegation-facing API categories added for `FBattleScreen` forwarding (state/control, setup/scenario, ship/weapon/defense selection, hex-click dispatch, movement/fire progression, renderer-facing tactical state accessors, and tactical report access).
...
- Keep the active runtime tactical GUI flow centered on `FBattleScreen`, `FBattleBoard`, and `FBattleDisplay` until later milestones complete the delegation rewrite; Milestones 5-8 documentation should still describe `FTacticalGame`, `ITacticalUI`, and `WXTacticalUI` as additive infrastructure rather than a fully model-driven live runtime path. Milestone 8 Subtask 1 expands the `FBattleScreen`/`FTacticalGame` forwarding surface for selection, placement, hex-click handling, movement/fire progression, and renderer-facing state queries, and Milestone 8 Subtask 2 moves `FBattleDisplay` fire/setup selection handling over that seam so the display now acts as a HUD/event translator for weapon selection, defense selection, fire-phase completion, and mine-placement setup/completion. `FBattleBoard` still retains its legacy board-state ownership and click semantics until the later Milestone 8 board subtask completes.
