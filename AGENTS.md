...
- Keep the active runtime tactical GUI flow on `FBattleScreen`, `FBattleBoard`, and `FBattleDisplay` until later milestones explicitly rewire delegation; Milestones 5-8 documentation should describe `FTacticalGame`, `ITacticalUI`, and `WXTacticalUI` as additive only and not yet the live wx runtime path, even though `FBattleScreen` now owns the delegation objects' lifecycle and `FTacticalGame` exposes the canonical state-owner API surface needed for later forwarding work. Milestone 8 Subtask 1 expands that forwarding surface for selection, placement, hex-click handling, movement/fire progression, and renderer-facing state queries, but it does not yet refactor `FBattleBoard` or `FBattleDisplay` into pure wx render/input translators.
