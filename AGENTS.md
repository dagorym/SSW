...
- `include/core/FSaveFormat.h` is the core-pure home for the shared save-format wire constants (`kSaveMagic`, `kSaveFormatVersion`, `kMaxSerializedStringBytes`). `FPObject::writeU32`/`readU32` are the fixed-width little-endian primitives for counts/IDs; `FPObject::writeString`/`readString` write/read a `writeU32`-prefixed length followed by the raw bytes (embedded NULs preserved) and validate the decoded length against `kMaxSerializedStringBytes` before allocating anything. New or changed `save()`/`load()` wire fields for counts and IDs should route through `writeU32`/`readU32` rather than the native-representation `write<T>`/`read<T>` templates, which remain byte-for-byte pass-throughs (now with a post-I/O stream-state check) for non-portable scalar fields. As of this writing `FPObject` and the strategic `FSystem`/`FJumpRoute`/`FMap` `save()`/`load()` call sites (P5-4) use these primitives, including `FJumpRoute`'s start/end system IDs, which are now real fixed-width `writeU32`/`readU32` fields resolved by `FMap::load` via `getSystem(id)` instead of the removed `(FSystem*)`-cast-plus-`0x0000FFFFL`-mask pointer-smuggling hack; the remaining strategic/ships/weapons/defenses `save()`/`load()` call sites still use the legacy `write<T>`/`read<T>` templates for counts/IDs pending later save-format-hardening subtasks (see `plans/phase5-save-format-hardening-plan.md`).
