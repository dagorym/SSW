...
- GUI tests: `tests/gui/*` — `StrategicGuiLiveTest`, `TacticalGuiLiveTest`, and `BattleSimGuiLiveTest` using `WXGuiTestHarness` helpers (`showModalWithAction(...)`, `runModalFunctionWithAction(...)`, `runVoidFunctionWithAction(...)`, `waitForTopLevelWindow(...)`, `waitForTopLevelWindowClosed(...)`, `waitForModalDialog(...)`). Key invariants:
  ...
  - Posted `ID_TacticalQuit` and `wxEVT_CLOSE_WINDOW` close vectors must leave battle screens hidden or absent; harness waits drive wx idle cleanup until windows disappear. When a `WXTacticalUI` child modal dialog (damage summary, ICM selection, or winner/info message) is active at close time, `closeBattleScreen()` calls `dismissActiveDialog()` on the owned `WXTacticalUI` before `Hide()`/`Show(false)` to prevent the duplicate `wxDialog::EndModal` assert in wxGTK (`src/gtk/dialog.cpp:193`). `WXTacticalUI` exposes `hasPendingDialog()` (returns `true` while a child `ShowModal` is live) and `dismissActiveDialog()` (calls `EndModal(wxID_CANCEL)` to clear `IsModal()`) as the supporting seam; `m_activeDialog` is set around each `ShowModal` call in `showMessage`, `showDamageSummary`, and `runICMSelection` and cleared on return. In tests, `wxEVT_CLOSE_WINDOW` is posted after `runVoidFunctionWithAction` returns (not from within the `ShowModal` action lambda) to avoid a `Destroy()`-during-`ShowModal` double-free that is specific to the non-modal test setup (a harness artifact absent in production, where `FBattleScreen` runs via `ShowModal` and `closeBattleScreen()` calls `EndModal` with no `Destroy()`); `hasPendingDialog()`/`dismissActiveDialog()` are driven directly from the action lambda to verify dismiss-API correctness while `ShowModal` is live, and a source-contract supplement locks the `dismissActiveDialog()` call in `closeBattleScreen()` to cover the integration point that cannot be reproduced in-harness without triggering the double-free.
